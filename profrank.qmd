...
```{r}
responsessta <- read_csv("~/sta323-final-combine/sampledata/STA/evaluations_responses.csv")

responsesstatsci <- read_csv("~/sta323-final-combine/sampledata/STATSCI/evaluations_responses.csv")
...
make_prop_table <- function(department,
  ...
  department_df <- department_df |>
    ...
    mutate(course_num = as.integer(str_extract(course_clean, "\\d+")))
  if (level == "undergrad") {
    department_df <- department_df |>
      filter(!is.na(course_num) & course_num < 500)
  } else if (level == "grad") {
    department_df <- department_df |>
      filter(!is.na(course_num) & course_num >= 500)
  }
